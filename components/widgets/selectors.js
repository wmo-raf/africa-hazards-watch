...
export const getLocationObj = createSelector(
  ...
  (location, title) => {
    return {
      ...location,
      locationLabel: location.type === "africa" ? "africa" : title,
      adminLevel: locationLevelToStr(location),
      locationLabelFull: location.type === "africa" ? "africa" : title,
    };
  }
);
...
export const getLocationData = createSelector(
  ...
  (locationObj, allLocationData, polynamesWhitelist) => {
    ...
    if (adminLevel === "adm0") {
      parent = { label: "africa", value: "africa" };
      children = adm1Data;
    ...
    const status = ["africa", "country", "wdpa"].includes(locationObj.type)
      ? "saved"
